...
public class StatsCollector extends PerformanceAnalyzerMetricsCollector {
    ...
    if (statsCollector == null) {
      synchronized (StatsCollector.class) {
        if (statsCollector == null) {
          statsCollector =
              new StatsCollector(
                  loadMetadata(PluginSettings.instance().getSettingValue(STATS_TYPE, STATS_TYPE)));
        }
      }
    }
  ...
  private static void logTimeMetrics(
    ...
    addEntry(
        "StartTime",
        String.format(Locale.ROOT, "%.3f", startTimeMillis / MILLISECONDS_TO_SECONDS_DIVISOR),
        builder);
