...
public abstract class OldGenRca<T extends ResourceFlowUnit<?>> extends Rca<T> {
  ...
  protected double getMaxHeapSizeOrDefault(final double defaultValue) {
    ...
    for (MetricFlowUnit heapMaxMetric : heapMaxMetrics) {
      ...
      double ret =
          SQLParsingUtil
              .readDataFromSqlResult(heapMaxMetric.getData(), AllMetrics.HeapDimension.MEM_TYPE.getField(), AllMetrics.GCType.HEAP.toString(), MetricsDB.MAX);
      if (Double.isNaN(ret)) {
  ...
  protected double getMaxOldGenSizeOrDefault(final double defaultValue) {
    ...
    for (MetricFlowUnit heapMaxMetric : heapMaxMetrics) {
      ...
      double ret =
          SQLParsingUtil
              .readDataFromSqlResult(heapMaxMetric.getData(), AllMetrics.HeapDimension.MEM_TYPE.getField(), AllMetrics.GCType.OLD_GEN.toString(), MetricsDB.MAX);
      if (Double.isNaN(ret)) {
        LOG.error("Failed to parse metric in FlowUnit from {}", heap_Max.getClass().getName());
      } else {
  ...
  protected int getFullGcEventsOrDefault(final double defaultValue) {
    ...
    for (MetricFlowUnit gcEventMetric : gcEventMetrics) {
      ...
      double ret =
          SQLParsingUtil.readDataFromSqlResult(gcEventMetric.getData(), AllMetrics.HeapDimension.MEM_TYPE.getField(), AllMetrics.GCType.TOT_FULL_GC.toString(), MetricsDB.MAX);
      if (Double.isNaN(ret)) {
  ...
  protected double getOldGenUsedOrDefault(final double defaultValue) {
    ...
    for (MetricFlowUnit heapUsedMetric : heapUsedMetrics) {
      ...
      double ret =
          SQLParsingUtil.readDataFromSqlResult(heapUsedMetric.getData(), AllMetrics.HeapDimension.MEM_TYPE.getField(), AllMetrics.GCType.OLD_GEN.toString(), MetricsDB.MAX);
      if (Double.isNaN(ret)) {
