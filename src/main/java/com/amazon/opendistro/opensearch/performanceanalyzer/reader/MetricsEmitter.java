...
public class MetricsEmitter {
  ...
  public static void emitWorkloadMetrics(
    ...
    db.createMetric(
        new Metric<Double>(AllMetrics.CommonMetric.LATENCY.toString(), 0d), LATENCY_TABLE_DIMENSIONS);
  ...
  public static void emitGarbageCollectionInfo(MetricsDB metricsDB,
    ...
    metricsDB.createMetric(new Metric<>(AllMetrics.GCInfoValue.GARBAGE_COLLECTOR_TYPE.toString(), 0d), dims);
    BatchBindStep handle =
        metricsDB.startBatchPut(new Metric<>(AllMetrics.GCInfoValue.GARBAGE_COLLECTOR_TYPE.toString(), 0d),
            dims);
    for (Record record : gcTypeRecords) {
      Optional<Object> memPoolObj =
          Optional.ofNullable(record.get(AllMetrics.GCInfoDimension.MEMORY_POOL.toString()));
      Optional<Object> collectorObj =
          Optional.ofNullable(record.get(AllMetrics.GCInfoDimension.COLLECTOR_NAME.toString()));
  ...
  public static void emitAdmissionControlMetrics(MetricsDB metricsDB, AdmissionControlSnapshot snapshot) {
    ...
    metricsDB.createMetric(new Metric<Double>(AllMetrics.AdmissionControlValue.REJECTION_COUNT.toString(), 0d), dims);
    BatchBindStep handle = metricsDB.startBatchPut(
            new Metric<Double>(AllMetrics.AdmissionControlValue.REJECTION_COUNT.toString(), 0d), dims);
