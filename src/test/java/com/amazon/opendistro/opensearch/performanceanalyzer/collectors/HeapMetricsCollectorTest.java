...
public class HeapMetricsCollectorTest extends AbstractCollectorTest {
  ...
  public void validateMetric(String metric) throws Exception {
    HeapMetricsCollector.HeapStatus heapStatus = mapper.readValue(metric, HeapMetricsCollector.HeapStatus.class);
    ...
    Assert.assertTrue(collectionCount >= 0 || collectionCount == HeapMetricsCollector.HeapStatus.UNDEFINED);
    long collectionTime = heapStatus.getCollectionTime();
    Assert.assertTrue(collectionTime >= 0 || collectionTime == HeapMetricsCollector.HeapStatus.UNDEFINED);
    ...
    Assert.assertTrue(max >= 0 || max == HeapMetricsCollector.HeapStatus.UNDEFINED || max == -1);
