...
public class CacheClearActionTest {
  ...
  public void setupClusterDetails() {
    ...
    ClusterDetailsEventProcessor.NodeDetails master =
        new ClusterDetailsEventProcessor.NodeDetails(AllMetrics.NodeRole.ELECTED_MASTER, "master", "127.0.0.3", true);
    ...
    dataNodeKeySet.add(new NodeKey(new InstanceDetails.Id(node1.getId()), new InstanceDetails.Ip(node1.getHostAddress())));
    dataNodeKeySet.add(new NodeKey(new InstanceDetails.Id(node2.getId()), new InstanceDetails.Ip(node2.getHostAddress())));
  }
  ...
  public void testBuildAction() {
    ...
    Assert.assertEquals(CacheClearAction.Builder.DEFAULT_COOL_OFF_PERIOD_IN_MILLIS, cacheClearAction.coolOffPeriodInMillis());
