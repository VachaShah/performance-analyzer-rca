...
public class JvmGenTuningPolicyTest {
  ...
  private void mockRcaIssues(int n, Resource resource) {
    ...
    HotNodeSummary nodeSummary = new HotNodeSummary(new InstanceDetails.Id("A"), new InstanceDetails.Ip("127.0.0.1"));
    for (int i = 0; i < n; i++) {
  ...
  private void mockCurrentRatio(double desiredRatio) {
    ...
    when(appContext.getDataNodeInstances()).thenReturn(Collections.singletonList(
        new InstanceDetails(AllMetrics.NodeRole.DATA, new InstanceDetails.Id("A"), new InstanceDetails.Ip("127.0.0.1"), false)));
