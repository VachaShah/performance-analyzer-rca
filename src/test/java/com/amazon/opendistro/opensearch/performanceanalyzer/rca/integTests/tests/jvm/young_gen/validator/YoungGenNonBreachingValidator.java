...
public class YoungGenNonBreachingValidator implements IValidator {
  ...
  private boolean checkPersistedAction(JsonArray array) {
    for (int i = 0; i < array.size(); i++) {
      ...
      Assert.assertNotEquals(HeapSizeIncreaseAction.NAME, object.get(PersistedAction.SQL_SCHEMA_CONSTANTS.ACTION_COL_NAME).getAsString());

      //validate no young gen action is emitted
      Assert.assertNotEquals(JvmGenAction.NAME, object.get(PersistedAction.SQL_SCHEMA_CONSTANTS.ACTION_COL_NAME).getAsString());
    }
    return true;
  }
}
