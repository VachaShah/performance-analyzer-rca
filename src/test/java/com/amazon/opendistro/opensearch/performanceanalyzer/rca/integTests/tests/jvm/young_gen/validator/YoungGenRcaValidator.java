...
public class YoungGenRcaValidator implements IValidator {
  ...
  boolean checkClusterRca(JsonObject object) {
    ...
    Assert.assertEquals(ResourceEnum.OLD_GEN.toString(),
        array.get(0).getAsJsonObject().get(HotResourceSummary.SQL_SCHEMA_CONSTANTS.RESOURCE_TYPE_COL_NAME).getAsString());
    Assert.assertEquals(MetricEnum.FULL_GC.toString(),
        array.get(0).getAsJsonObject().get(HotResourceSummary.SQL_SCHEMA_CONSTANTS.RESOURCE_METRIC_COL_NAME).getAsString());
