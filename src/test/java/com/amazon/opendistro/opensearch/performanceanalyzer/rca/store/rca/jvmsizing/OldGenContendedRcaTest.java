...
public class OldGenContendedRcaTest {
  ...
  private final InstanceDetails currentInstance = new InstanceDetails(new InstanceDetails.Id("nodeId"),
      new InstanceDetails.Ip("1.2.3.4"), 0);
  ...
  public void oneDependentRcaUnhealthy() {
    when(mockOldGenOccupancyRca.getFlowUnits()).thenReturn(Collections
        .singletonList(new ResourceFlowUnit<>(System.currentTimeMillis(), new ResourceContext(
            Resources.State.UNHEALTHY), new HotResourceSummary(ResourceUtil.OLD_GEN_HEAP_USAGE, 0d, 0d, 0))));
    when(mockOldGenReclamationRca.getFlowUnits()).thenReturn(Collections
        .singletonList(new ResourceFlowUnit<>(System.currentTimeMillis(), new ResourceContext(
            Resources.State.HEALTHY), new HotResourceSummary(ResourceUtil.FULL_GC_EFFECTIVENESS, 0d, 0d, 0))));
    ...
    when(mockOldGenOccupancyRca.getFlowUnits()).thenReturn(Collections
        .singletonList(new ResourceFlowUnit<>(System.currentTimeMillis(), new ResourceContext(
            Resources.State.HEALTHY), new HotResourceSummary(ResourceUtil.OLD_GEN_HEAP_USAGE, 0d, 0d, 0))));
    when(mockOldGenReclamationRca.getFlowUnits()).thenReturn(Collections
        .singletonList(new ResourceFlowUnit<>(System.currentTimeMillis(), new ResourceContext(
            Resources.State.UNHEALTHY), new HotResourceSummary(ResourceUtil.FULL_GC_EFFECTIVENESS, 0d, 0d,
            0))));
  ...
  public void testContendedOldGen() {
    ...
    Assert.assertEquals(currentInstance.getInstanceIp(), flowUnit.getSummary().getHostAddress());
  }
