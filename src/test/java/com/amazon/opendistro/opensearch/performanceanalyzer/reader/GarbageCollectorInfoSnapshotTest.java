...
public class GarbageCollectorInfoSnapshotTest {
  ...
  public void testReadGcSnapshot() throws Exception {
    ...
    Assert.assertEquals(TEST_MEM_POOL, result.get(0).get(GarbageCollectorInfoSnapshot.Fields.MEMORY_POOL.toString()));
    Assert.assertEquals(COLLECTOR_NAME, result.get(0).get(GarbageCollectorInfoSnapshot.Fields.COLLECTOR_NAME.toString()));
  }
