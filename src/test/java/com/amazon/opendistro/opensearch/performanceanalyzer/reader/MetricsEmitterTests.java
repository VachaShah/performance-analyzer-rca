...
public class MetricsEmitterTests extends AbstractReaderTests {
  ...
  public void testMetricsEmitter() throws Exception {
    ...
    res =
        db.queryMetric(
            Arrays.asList(AllMetrics.OSMetrics.PAGING_RSS.toString(), AllMetrics.OSMetrics.CPU_UTILIZATION.toString()),
    ...
    Double cpu =
        Double.parseDouble(res.get(0).get(AllMetrics.OSMetrics.CPU_UTILIZATION.toString()).toString());
    db.remove();
  ...
  public void testHttpMetricsEmitter() throws Exception {
    ...
    Result<Record> res =
        db.queryMetric(
            Arrays.asList(
                AllMetrics.CommonMetric.LATENCY.toString(), AllMetrics.HttpMetric.HTTP_TOTAL_REQUESTS.toString()),
            Arrays.asList("avg", "sum"),
            Arrays.asList(HttpRequestMetricsSnapshot.Fields.OPERATION.toString()));

    Float latency = Float.parseFloat(res.get(0).get(AllMetrics.CommonMetric.LATENCY.toString()).toString());
    db.remove();
  ...
  public void testWorkloadMetricsEmitter() throws Exception {
    ...
    Double bulkDocs =
        Double.parseDouble(res.get(0).get(AllMetrics.ShardBulkMetric.DOC_COUNT.toString()).toString());
    Double shardOps =
        Double.parseDouble(
            res.get(0).get(AllMetrics.ShardOperationMetric.SHARD_OP_COUNT.toString()).toString());
    db.remove();
  ...
  public void testEmitNodeMetrics() throws Exception {
    ...
    MemoryDBSnapshot tcpSnap = new MemoryDBSnapshot(conn, AllMetrics.MetricName.TCP_METRICS, 5001L);
    ...
    Result<Record> res =
        db.queryMetric(
            Arrays.asList(
                AllMetrics.TCPValue.Net_TCP_NUM_FLOWS.toString(), AllMetrics.TCPValue.Net_TCP_SSTHRESH.toString()),
    ...
    for (int i = 0; i < 6; i++) {
      ...
      Double numFlows =
          Double.parseDouble(record0.get(AllMetrics.TCPValue.Net_TCP_NUM_FLOWS.toString()).toString());

      assertThat(
          numFlows.doubleValue(), anyOf(closeTo(24, 0.001), closeTo(23, 0.001), closeTo(4, 0.001)));

      Double ssThresh =
          Double.parseDouble(record0.get(AllMetrics.TCPValue.Net_TCP_SSTHRESH.toString()).toString());
  ...
  public void testEmitGCTypeMetric() throws Exception {
    ...
    Result<Record> result = metricsDB.queryMetric(AllMetrics.GCInfoValue.GARBAGE_COLLECTOR_TYPE.toString());

    assertEquals(1, result.size());
    Assert.assertEquals(memPool, result.get(0).get(AllMetrics.GCInfoDimension.MEMORY_POOL.getField()));
    Assert.assertEquals(collectorName, result.get(0).get(AllMetrics.GCInfoDimension.COLLECTOR_NAME.getField()));
  }
