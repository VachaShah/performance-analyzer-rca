...
public class ReaderMetricsProcessorTests extends AbstractReaderTests {
  ...
  private NavigableMap<Long, MemoryDBSnapshot> setUpAligningWindow(long lastUpdateTime3)
    ...
    MemoryDBSnapshot masterPendingSnap1 =
        new MemoryDBSnapshot(conn, AllMetrics.MetricName.MASTER_PENDING, 6001L);
    ...
    MemoryDBSnapshot masterPendingSnap2 =
        new MemoryDBSnapshot(conn, AllMetrics.MetricName.MASTER_PENDING, 11001L);
    ...
    MemoryDBSnapshot masterPendingSnap3 =
        new MemoryDBSnapshot(conn, AllMetrics.MetricName.MASTER_PENDING, 16001L);
  ...
  public void testAlignNodeMetrics() throws Exception {
    ...
    Field<Double> valueField =
        DSL.field(AllMetrics.MasterPendingValue.MASTER_PENDING_QUEUE_SIZE.toString(), Double.class);
  ...
  public void testEmitNodeMetrics() throws Exception {
    ...
    spyMp.putNodeMetricsMap(AllMetrics.MetricName.MASTER_PENDING, metricMap);
    ...
    Result<Record> res = db.queryMetric(AllMetrics.MasterPendingValue.MASTER_PENDING_QUEUE_SIZE.toString());
  ...
  public void testWriterWindowEndsBeforeReaderWindow() throws Exception {
    ...
    assertTrue(!db.metricExists(AllMetrics.MasterPendingValue.MASTER_PENDING_QUEUE_SIZE.toString()));
    db.remove();
  }
  ...
  public void testReadBatchMetricsEnabledFromConf() throws Exception {
    ...
    Path batchMetricsEnabledConfFile = Paths.get(Util.DATA_DIR, ReaderMetricsProcessor.BATCH_METRICS_ENABLED_CONF_FILE);
  ...
  public void testTrimOldSnapshots() throws Exception {
    deleteAll();
    Files.createDirectories(Paths.get(Util.DATA_DIR));
    Files.write(Paths.get(Util.DATA_DIR, ReaderMetricsProcessor.BATCH_METRICS_ENABLED_CONF_FILE), Boolean.toString(false).getBytes());
    ...
    do {
      ...
      Files.write(Paths.get(Util.DATA_DIR, ReaderMetricsProcessor.BATCH_METRICS_ENABLED_CONF_FILE), Boolean.toString(true).getBytes());
      for (int i = 0; i < 3; i++) {
        mp.processMetrics(rootLocation, currentTimestamp);
        mp.trimOldSnapshots();
        mp.trimOldMetricsDBFiles();
