...
public class HighHeapUsageYoungGenRcaTest {
  ...
  private void mockFlowUnits(double heapUsageVal, double minorGcTime, double fullGcTime, double fullGcEvents) {
    heap_Used.createTestFlowUnits(columnName, Arrays.asList(AllMetrics.GCType.OLD_GEN.toString(), String.valueOf(heapUsageVal * CONVERT_MEGABYTES_TO_BYTES)));
    List<String> youngGcRow = Arrays.asList(AllMetrics.GCType.TOT_YOUNG_GC.toString(), String.valueOf(minorGcTime));
    List<String> fullGcRow = Arrays.asList(AllMetrics.GCType.TOT_FULL_GC.toString(), String.valueOf(fullGcTime));
    gc_Collection_Time.createTestFlowUnitsWithMultipleRows(columnName, Lists.newArrayList(youngGcRow, fullGcRow));
    gc_Collection_Event.createTestFlowUnits(columnName, Arrays.asList(AllMetrics.GCType.OLD_GEN.toString(), String.valueOf(fullGcEvents)));
    gc_Type.createTestFlowUnits(Arrays.asList(AllMetrics.GCInfoDimension.MEMORY_POOL.toString(),
        AllMetrics.GCInfoDimension.COLLECTOR_NAME.toString()), Arrays.asList(AllMetrics.GCType.OLD_GEN.toString(), CMS_COLLECTOR));
  }
